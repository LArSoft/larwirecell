...
WIRECELL_FACTORY(QLMatching, WireCell::QLMatch::QLMatching,
                 WireCell::INamed,
...
WireCell::QLMatch::QLMatching::QLMatching()
    : Aux::Logger("QLMatching", "matching")
{
}
...
bool WireCell::QLMatch::QLMatching::operator()(const input_vector& invec, output_pointer& out)
{
    ...
    if (!m_bee_dir.empty()) {
        ...
        QLMatch::dump_bee_3d(*root_live.get(), String::format("%s/%d-img-apa%d.json", sub_dir, charge_ident, m_anode->ident()));
        QLMatch::dump_bee_flash(invec[1], String::format("%s/%d-op-apa%d.json", sub_dir, charge_ident, m_anode->ident()));
    }
