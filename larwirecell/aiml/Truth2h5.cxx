...
namespace {
  hid_t neutrino_type()
  {
    ...
    H5Tinsert(t, "nu_intType", HOFFSET(WireCell::AIML::Truth2h5::NeutrinoInfo, intType), H5T_NATIVE_INT);
    H5Tinsert(t, "nu_energy", HOFFSET(WireCell::AIML::Truth2h5::NeutrinoInfo, energy), H5T_NATIVE_DOUBLE);
    H5Tinsert(t, "nu_vertex_x", HOFFSET(WireCell::AIML::Truth2h5::NeutrinoInfo, vx), H5T_NATIVE_DOUBLE);
    H5Tinsert(t, "nu_vertex_y", HOFFSET(WireCell::AIML::Truth2h5::NeutrinoInfo, vy), H5T_NATIVE_DOUBLE);
    H5Tinsert(t, "nu_vertex_z", HOFFSET(WireCell::AIML::Truth2h5::NeutrinoInfo, vz), H5T_NATIVE_DOUBLE);
    return t;
  }
} // namespace
...
void AIML::Truth2h5::ensure_file()
{
  ...
  if (m_file < 0) {
    ...
    if (m_file < 0) {
      log->error("Truth2h5 failed to open output file {}", m_output_file);
    }
  }
}
...
void AIML::Truth2h5::write_event(const IFrame& frame)
{
  ...
  hid_t dset = H5Dcreate2(m_file, dset_name.c_str(), dtype, dataspace, lcpl, H5P_DEFAULT, H5P_DEFAULT);
  H5Pclose(lcpl);
  if (dset < 0) {
  ...
  if (status < 0) {
    log->warn("Truth2h5 failed to write dataset {}", dset_name);
  }
